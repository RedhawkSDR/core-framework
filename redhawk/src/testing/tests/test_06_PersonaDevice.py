    ...
    def allocate(self):
        ...
        for device in self._devMgr._get_registeredDevices():
            if "ProgrammableDevice_1" not in device._get_identifier():
                ...
                if deviceNumber == 1:
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[1]._get_usageState()),"IDLE")
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[1]._get_adminState()),"UNLOCKED")
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[2]._get_usageState()),"BUSY")
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[2]._get_adminState()),"LOCKED")
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[3]._get_usageState()),"BUSY")
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[3]._get_adminState()),"LOCKED")
                elif deviceNumber == 2:
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[1]._get_usageState()),"BUSY")
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[1]._get_adminState()),"LOCKED")
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[2]._get_usageState()),"IDLE")
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[2]._get_adminState()),"UNLOCKED")
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[3]._get_usageState()),"BUSY")
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[3]._get_adminState()),"LOCKED")
                elif deviceNumber == 3:
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[1]._get_usageState()),"BUSY")
                    ...
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[2]._get_usageState()),"BUSY")
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[2]._get_adminState()),"LOCKED")
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[3]._get_usageState()),"IDLE")
                    self.assertEqual(str(self._devMgr._get_registeredDevices()[3]._get_adminState()),"UNLOCKED")
...
class PPTest(scatest.CorbaTestCase):
    ...
    def test_CPP_ProgrammableAndPersona(self):
        ...
        self.assertEqual(devs[0]._get_identifier(), 'agg_test:base_programmable_1')
        self.assertEqual(devs[1]._get_identifier(), 'agg_test:base_persona_1')
        self.assertEqual(devs[0]._get_compositeDevice(), None)
        self.assertEqual(devs[1]._get_compositeDevice()._get_identifier(), 'agg_test:base_programmable_1')
