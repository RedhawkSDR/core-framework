...
from py_dev_base import *

class py_dev_i(py_dev_base):
    ...
    def initialize(self):
        ...
        """
        py_dev_base.initialize(self)
        self.devmgr_id = self.getDeviceManager().getRef()._get_identifier()
        if self.getDomainManager().getRef() != None:
            self.dom_id = self.getDomainManager().getRef()._get_identifier()
        else:
            self.dom_id = ""
