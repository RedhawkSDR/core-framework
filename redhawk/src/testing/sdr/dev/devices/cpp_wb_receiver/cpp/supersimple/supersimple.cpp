...
supersimple_i::supersimple_i(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl) :
    supersimple_base(devMgr_ior, id, lbl, sftwrPrfl)
{
}
...
supersimple_i::supersimple_i(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, CF::Properties capacities) :
    supersimple_base(devMgr_ior, id, lbl, sftwrPrfl, capacities)
{
}

supersimple_i::supersimple_i(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, CF::Properties capacities, char *compDev) :
    supersimple_base(devMgr_ior, id, lbl, sftwrPrfl, capacities, compDev)
{
}

supersimple_i::~supersimple_i()
{
}

void supersimple_i::constructor()
{
    /***********************************************************************************
...
**************************************************************************/
void supersimple_i::updateUsageState()
{
}
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void supersimple_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &supersimple_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &supersimple_i::<callback method>)
        in the constructor.
        ...
        //Add to supersimple.cpp
        supersimple_i::supersimple_i(const char *uuid, const char *label) :
            supersimple_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &supersimple_i::scaleChanged);
            addPropertyListener(status, this, &supersimple_i::statusChanged);
        }

        void supersimple_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void supersimple_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            RH_DEBUG(this->_baseLog, "status changed");
        }
            
        //Add to supersimple.h
    ...
    Allocation:
        ...
        bool supersimple_i::my_alloc_fn(const std::string &value)
        {
            // perform logic
            return true; // successful allocation
        }
        void supersimple_i::my_dealloc_fn(const std::string &value)
        {
            // perform logic
        }
        ...
        this->setAllocationImpl(my_alloc, this, &supersimple_i::my_alloc_fn, &supersimple_i::my_dealloc_fn);
