...
import logging
import traceback
from svc_port_base import *

class svc_port_i(svc_port_base):
    ...
    def constructor(self):
        ...
        self.registerSvc1=True
        self.registerSvc2=True

    def releaseObject(self):
        # model releaseObject is not called if the signal is received by this process before the device manager
        pass
    ...
    def process(self):
        ...
        Messages:
            ...
            self.port_input.registerMessage("my_msg", svc_port_i.MyMsg, self.msg_callback)
    ...
    Accessing the Application and Domain Manager:
        ...
        Example:
            ...
            if self.increaseAmplitude:
                ...
                scale = 1.0      std::cout<<"........ done with shutdown"<<std::endl;
        ...
        if self.registerSvc1:
            self._devmgr.registerService(self.port_svc1_in._this(), self.svc_1)
            self.registerSvc1=False

        if self.registerSvc2:
            self._devmgr.registerService(self.port_svc2_in._this(), self.svc_2)
            self.registerSvc2=False

        if not self.registerSvc1 and not self.registerSvc2:
            return FINISH
        return NOOP
