...
PREPARE_LOGGING(devcpp_i)

devcpp_i::devcpp_i(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl) :
    devcpp_base(devMgr_ior, id, lbl, sftwrPrfl)
{
}

devcpp_i::devcpp_i(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, char *compDev) :
    devcpp_base(devMgr_ior, id, lbl, sftwrPrfl, compDev)
{
}

devcpp_i::devcpp_i(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, CF::Properties capacities) :
    devcpp_base(devMgr_ior, id, lbl, sftwrPrfl, capacities)
{
}

devcpp_i::devcpp_i(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, CF::Properties capacities, char *compDev) :
    devcpp_base(devMgr_ior, id, lbl, sftwrPrfl, capacities, compDev)
{
}
...
void devcpp_i::set_foobar( int value ) {
  LOG_INFO(devcpp_i, " current value " << foobar );

  LOG_INFO(devcpp_i, " new value to set " << value );
}
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void devcpp_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &devcpp_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &devcpp_i::<callback method>)
        in the constructor.
        ...
        //Add to devcpp.cpp
        devcpp_i::devcpp_i(const char *uuid, const char *label) :
            devcpp_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &devcpp_i::scaleChanged);
            addPropertyListener(status, this, &devcpp_i::statusChanged);
        }

        void devcpp_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(devcpp_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void devcpp_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(devcpp_i, "status changed");
        }
    ...
    Allocation:
        ...
        bool devcpp_i::my_alloc_fn(const std::string &value)
        {
            // perform logic
            return true; // successful allocation
        }
        void devcpp_i::my_dealloc_fn(const std::string &value)
        {
            // perform logic
        }
        ...
        this->setAllocationImpl(my_alloc, this, &devcpp_i::my_alloc_fn, &devcpp_i::my_dealloc_fn);
        
        

************************************************************************************************/
int devcpp_i::serviceFunction()
{
    LOG_TRACE(devcpp_i, "serviceFunction() example TRACE log message");
    LOG_DEBUG(devcpp_i, "serviceFunction() example DEBUG log message");
    LOG_INFO(devcpp_i, "serviceFunction() example INFO log message");
    LOG_ERROR(devcpp_i, "serviceFunction() example ERROR log message");
    LOG_WARN(devcpp_i, "serviceFunction() example WARN log message");
    LOG_FATAL(devcpp_i, "serviceFunction() example FATAL log message");
    return NOOP;
}
