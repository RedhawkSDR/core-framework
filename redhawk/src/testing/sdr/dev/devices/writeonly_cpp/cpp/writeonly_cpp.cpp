...
#include "writeonly_cpp.h"

PREPARE_LOGGING(writeonly_cpp_i)

writeonly_cpp_i::writeonly_cpp_i(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl) :
    writeonly_cpp_base(devMgr_ior, id, lbl, sftwrPrfl)
{
}

writeonly_cpp_i::writeonly_cpp_i(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, char *compDev) :
    writeonly_cpp_base(devMgr_ior, id, lbl, sftwrPrfl, compDev)
{
}

writeonly_cpp_i::writeonly_cpp_i(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, CF::Properties capacities) :
    writeonly_cpp_base(devMgr_ior, id, lbl, sftwrPrfl, capacities)
{
}

writeonly_cpp_i::writeonly_cpp_i(char *devMgr_ior, char *id, char *lbl, char *sftwrPrfl, CF::Properties capacities, char *compDev) :
    writeonly_cpp_base(devMgr_ior, id, lbl, sftwrPrfl, capacities, compDev)
{
}

writeonly_cpp_i::~writeonly_cpp_i()
{
}

void writeonly_cpp_i::constructor()
{
    /***********************************************************************************
...
**************************************************************************/
void writeonly_cpp_i::updateUsageState()
{
}
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void writeonly_cpp_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &writeonly_cpp_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &writeonly_cpp_i::<callback method>)
        in the constructor.
        ...
        //Add to writeonly_cpp.cpp
        writeonly_cpp_i::writeonly_cpp_i(const char *uuid, const char *label) :
            writeonly_cpp_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &writeonly_cpp_i::scaleChanged);
            addPropertyListener(status, this, &writeonly_cpp_i::statusChanged);
        }

        void writeonly_cpp_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(writeonly_cpp_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void writeonly_cpp_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(writeonly_cpp_i, "status changed");
        }
            
        //Add to writeonly_cpp.h
    ...
    Allocation:
        ...
        bool writeonly_cpp_i::my_alloc_fn(const std::string &value)
        {
            // perform logic
            return true; // successful allocation
        }
        void writeonly_cpp_i::my_dealloc_fn(const std::string &value)
        {
            // perform logic
        }
        ...
        this->setAllocationImpl(my_alloc, this, &writeonly_cpp_i::my_alloc_fn, &writeonly_cpp_i::my_dealloc_fn);
        
        

************************************************************************************************/
int writeonly_cpp_i::serviceFunction()
{
    LOG_DEBUG(writeonly_cpp_i, "serviceFunction() example log message");
