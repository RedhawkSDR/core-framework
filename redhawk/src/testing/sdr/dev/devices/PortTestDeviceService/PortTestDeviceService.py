...
class testOut_i(CF__POA.Port):
    ...
    def connectPort(self, connection, connectionId):
        port = connection._narrow(CF__POA.PropertySet)
    ...
    def query(self, propsIn):
        propsOut = []
        for propset in self.outPorts:
            propsOut += self.outPorts[propset].query(propsIn)
        return propsOut
...
class PortDevice_impl(CF__POA.ExecutableDevice, ExecutableDevice):
    ...
    def runTest(self, testid, properties):
        if testid == 0:
            ...
            self.testRun = True
            return self.toTest.query([])
        elif testid == 4:
            return [CF.DataType(id='testRun', value=any.to_any(self.testRun))]
        else:
