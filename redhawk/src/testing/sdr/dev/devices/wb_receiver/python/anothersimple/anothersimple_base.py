...
class anothersimple_base(CF__POA.AggregatePlainDevice, Device, AggregateDevice, ThreadedComponent, DynamicComponent):
        ...
        def __init__(self, devmgr, uuid, label, softwareProfile, compositeDevice, execparams):
            ...
            self.port_qed = bulkio.InFloatPort("qed", maxsize=self.DEFAULT_QUEUE_SIZE)
            self.port_qed._portLog = self._baseLog.getChildLogger('qed', 'ports')
        ...
        port_qed = providesport(name="qed",
                                repid="IDL:BULKIO/dataFloat:1.0",
                                type_="control")

        port_xyz = usesport(name="xyz",
                            repid="IDL:BULKIO/dataFloat:1.0",
                            type_="control")
