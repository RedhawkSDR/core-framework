...
PREPARE_LOGGING(zero_length_i)

zero_length_i::zero_length_i(const char *uuid, const char *label) :
    zero_length_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

zero_length_i::~zero_length_i()
{
}

void zero_length_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void zero_length_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &zero_length_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &zero_length_i::<callback method>)
        in the constructor.
        ...
        //Add to zero_length.cpp
        zero_length_i::zero_length_i(const char *uuid, const char *label) :
            zero_length_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &zero_length_i::scaleChanged);
            addPropertyListener(status, this, &zero_length_i::statusChanged);
        }

        void zero_length_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(zero_length_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void zero_length_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(zero_length_i, "status changed");
        }
            
        //Add to zero_length.h
...
************************************************************************************************/
int zero_length_i::serviceFunction()
{
    LOG_DEBUG(zero_length_i, "serviceFunction() example log message");
