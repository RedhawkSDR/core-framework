...
PREPARE_LOGGING(time_cp_now_i)

time_cp_now_i::time_cp_now_i(const char *uuid, const char *label) :
    time_cp_now_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.
}

time_cp_now_i::~time_cp_now_i()
{
}

void time_cp_now_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void time_cp_now_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &time_cp_now_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &time_cp_now_i::<callback method>)
        in the constructor.
        ...
        //Add to time_cp_now.cpp
        time_cp_now_i::time_cp_now_i(const char *uuid, const char *label) :
            time_cp_now_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &time_cp_now_i::scaleChanged);
            addPropertyListener(status, this, &time_cp_now_i::statusChanged);
        }

        void time_cp_now_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(time_cp_now_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void time_cp_now_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(time_cp_now_i, "status changed");
        }
            
        //Add to time_cp_now.h
...
************************************************************************************************/
int time_cp_now_i::serviceFunction()
{
    LOG_DEBUG(time_cp_now_i, "serviceFunction() example log message");
