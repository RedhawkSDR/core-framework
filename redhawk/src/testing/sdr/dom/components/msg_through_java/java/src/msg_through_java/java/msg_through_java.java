 ...
 */
public class msg_through_java extends msg_through_java_base {
    /**
    ...
    {
        this.port_input.registerMessage("foo", new callback(this));
    }
    public class callback implements EventCallback<foo_struct> {
        
        private msg_through_java parent;
        public Object handlingMessagesLock;
        
        public callback(msg_through_java _parent) {
            this.parent = _parent;
        ...
        public void message(String id, Any msg) {
            synchronized (this.handlingMessagesLock) {
                ...
                foo_struct msg_container = new foo_struct();
                for (final DataType msg_item : msg_set) {
                    if (msg_item.id.equals("a")) {
                        msg_container.a.setValue(msg_item.value.extract_string());
                    }
                    if (msg_item.id.equals("b")) {
                        msg_container.b.setValue(msg_item.value.extract_string());
                    }
                    if (msg_item.id.equals("c")) {
                        msg_container.c.setValue(msg_item.value.extract_string());
                    }
                }
        ...
        public void messageReceived(String id, foo_struct msg) {
            synchronized (this.handlingMessagesLock) {
                this.parent.port_output.sendMessage(msg);
            }
        }
    }
