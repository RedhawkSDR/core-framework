...
# AUTO-GENERATED
#
# Source: FailableComponent.spd.xml
from ossie.resource import start_component
import logging

from FailableComponent_base import *

class FailableComponent_i(FailableComponent_base):
    ...
    def initialize(self):
        ...
        if self.fail_initialize:
            raise CF.LifeCycle.InitializeError(['Test failure'])
        FailableComponent_base.initialize(self)

    def releaseObject(self):
        if self.fail_release:
            raise CF.LifeCycle.ReleaseError(['Test failure'])
        FailableComponent_base.releaseObject(self)

    def start(self):
        if self.fail_start:
            raise CF.Resource.StartError(CF.CF_NOTSET, "Test failure")
        FailableComponent_base.start(self)
            
    def stop(self):
        if self.fail_stop:
            raise CF.Resource.StopError(CF.CF_NOTSET, "Test failure")
        FailableComponent_base.stop(self)
