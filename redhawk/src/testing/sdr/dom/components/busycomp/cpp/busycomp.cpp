...
PREPARE_LOGGING(busycomp_i)

busycomp_i::busycomp_i(const char *uuid, const char *label) :
    busycomp_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

busycomp_i::~busycomp_i()
{
}

void busycomp_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void busycomp_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &busycomp_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &busycomp_i::<callback method>)
        in the constructor.
        ...
        busycomp_i::busycomp_i(const char *uuid, const char *label) :
            busycomp_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &busycomp_i::scaleChanged);
            addPropertyListener(status, this, &busycomp_i::statusChanged);
        }

        void busycomp_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(busycomp_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void busycomp_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(busycomp_i, "status changed");
        }
...
************************************************************************************************/
int busycomp_i::serviceFunction()
{
    LOG_DEBUG(busycomp_i, "serviceFunction() example log message");
