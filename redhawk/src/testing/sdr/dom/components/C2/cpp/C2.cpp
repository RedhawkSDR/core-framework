...
PREPARE_LOGGING(C2_i)

C2_i::C2_i(const char *uuid, const char *label) :
    C2_base(uuid, label)
{
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void C2_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &C2_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &C2_i::<callback method>)
        in the constructor.
        ...
        C2_i::C2_i(const char *uuid, const char *label) :
            C2_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &C2_i::scaleChanged);
            addPropertyListener(status, this, &C2_i::statusChanged);
        }

        void C2_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(C2_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void C2_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(C2_i, "status changed");
        }
