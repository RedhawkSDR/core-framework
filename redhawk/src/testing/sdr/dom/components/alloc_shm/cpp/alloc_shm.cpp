...
PREPARE_LOGGING(alloc_shm_i)

alloc_shm_i::alloc_shm_i(const char *uuid, const char *label) :
...
void alloc_shm_i::constructor()
{
    ...
    redhawk::buffer<float> outputData(1000000);
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void alloc_shm_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &alloc_shm_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &alloc_shm_i::<callback method>)
        in the constructor.
        ...
        //Add to alloc_shm.cpp
        alloc_shm_i::alloc_shm_i(const char *uuid, const char *label) :
            alloc_shm_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &alloc_shm_i::scaleChanged);
            addPropertyListener(status, this, &alloc_shm_i::statusChanged);
        }

        void alloc_shm_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void alloc_shm_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
...
************************************************************************************************/
int alloc_shm_i::serviceFunction()
{
