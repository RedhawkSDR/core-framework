...
PREPARE_LOGGING(timeprop_cpp_i)

timeprop_cpp_i::timeprop_cpp_i(const char *uuid, const char *label) :
    timeprop_cpp_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

timeprop_cpp_i::~timeprop_cpp_i()
{
}

void timeprop_cpp_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void timeprop_cpp_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &timeprop_cpp_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &timeprop_cpp_i::<callback method>)
        in the constructor.
        ...
        //Add to timeprop_cpp.cpp
        timeprop_cpp_i::timeprop_cpp_i(const char *uuid, const char *label) :
            timeprop_cpp_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &timeprop_cpp_i::scaleChanged);
            addPropertyListener(status, this, &timeprop_cpp_i::statusChanged);
        }

        void timeprop_cpp_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(timeprop_cpp_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void timeprop_cpp_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(timeprop_cpp_i, "status changed");
        }
            
        //Add to timeprop_cpp.h
...
************************************************************************************************/
int timeprop_cpp_i::serviceFunction()
{
    LOG_DEBUG(timeprop_cpp_i, "serviceFunction() example log message");
