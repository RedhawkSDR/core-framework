 ...
 */
#ifndef CPPCALLBACKS_IMPL_BASE_H
#define CPPCALLBACKS_IMPL_BASE_H
...
class CppCallbacks_base : public Resource_impl
{
    public:
        CppCallbacks_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<CppCallbacks_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::ULong count;
        std::vector<std::complex<float> > constellation;
        std::vector<std::string> callbacks_run;
        station_struct station;
        std::vector<endpoint_struct> servers;
