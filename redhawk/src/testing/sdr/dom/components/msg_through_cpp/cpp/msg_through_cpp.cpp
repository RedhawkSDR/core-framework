...
PREPARE_LOGGING(msg_through_cpp_i)

msg_through_cpp_i::msg_through_cpp_i(const char *uuid, const char *label) :
    msg_through_cpp_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

msg_through_cpp_i::~msg_through_cpp_i()
{
}

void msg_through_cpp_i::constructor()
{
    /***********************************************************************************
     This is the RH constructor. All properties are properly initialized before this function is called 
    ***********************************************************************************/
	this->input->registerMessage("foo", this, &msg_through_cpp_i::msg_callback);
}

void msg_through_cpp_i::msg_callback(const std::string& id, const foo_struct& msg)
{
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void msg_through_cpp_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &msg_through_cpp_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &msg_through_cpp_i::<callback method>)
        in the constructor.
        ...
        //Add to msg_through_cpp.cpp
        msg_through_cpp_i::msg_through_cpp_i(const char *uuid, const char *label) :
            msg_through_cpp_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &msg_through_cpp_i::scaleChanged);
            addPropertyListener(status, this, &msg_through_cpp_i::statusChanged);
        }

        void msg_through_cpp_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(msg_through_cpp_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void msg_through_cpp_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(msg_through_cpp_i, "status changed");
        }
            
        //Add to msg_through_cpp.h
...
************************************************************************************************/
int msg_through_cpp_i::serviceFunction()
{
    LOG_DEBUG(msg_through_cpp_i, "serviceFunction() example log message");
