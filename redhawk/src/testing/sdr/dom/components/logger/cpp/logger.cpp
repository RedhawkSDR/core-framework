...
PREPARE_LOGGING(logger_i)

logger_i::logger_i(const char *uuid, const char *label) :
    logger_base(uuid, label)
{
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void logger_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &logger_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &logger_i::<callback method>)
        in the constructor.
        ...
        logger_i::logger_i(const char *uuid, const char *label) :
            logger_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &logger_i::scaleChanged);
            addPropertyListener(status, this, &logger_i::statusChanged);
        }

        void logger_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(logger_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void logger_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(logger_i, "status changed");
        }
