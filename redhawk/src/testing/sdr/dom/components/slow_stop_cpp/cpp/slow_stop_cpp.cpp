...
PREPARE_LOGGING(slow_stop_cpp_i)

slow_stop_cpp_i::slow_stop_cpp_i(const char *uuid, const char *label) :
    slow_stop_cpp_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

slow_stop_cpp_i::~slow_stop_cpp_i()
{
}

void slow_stop_cpp_i::constructor()
{
    /***********************************************************************************
     This is the RH constructor. All properties are properly initialized before this function is called 
    ***********************************************************************************/
    this->setThreadDelay(10);
}
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void slow_stop_cpp_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &slow_stop_cpp_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &slow_stop_cpp_i::<callback method>)
        in the constructor.
        ...
        //Add to slow_stop_cpp.cpp
        slow_stop_cpp_i::slow_stop_cpp_i(const char *uuid, const char *label) :
            slow_stop_cpp_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &slow_stop_cpp_i::scaleChanged);
            addPropertyListener(status, this, &slow_stop_cpp_i::statusChanged);
        }

        void slow_stop_cpp_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(slow_stop_cpp_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void slow_stop_cpp_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(slow_stop_cpp_i, "status changed");
        }
            
        //Add to slow_stop_cpp.h
...
************************************************************************************************/
int slow_stop_cpp_i::serviceFunction()
{
    LOG_DEBUG(slow_stop_cpp_i, "serviceFunction() example log message");
