...
void DomainEventReader_i::increment_add_events()
{
    num_add_events++;
}

void DomainEventReader_i::increment_remove_events()
{
    num_remove_events++;
}
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void DomainEventReader_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &DomainEventReader_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &DomainEventReader_i::<callback method>)
        in the constructor.
        ...
        //Add to DomainEventReader.cpp
        DomainEventReader_i::DomainEventReader_i(const char *uuid, const char *label) :
            DomainEventReader_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &DomainEventReader_i::scaleChanged);
            addPropertyListener(status, this, &DomainEventReader_i::statusChanged);
        }

        void DomainEventReader_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void DomainEventReader_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            RH_DEBUG(this->_baseLog, "status changed");
        }
            
        //Add to DomainEventReader.h
...
************************************************************************************************/
int DomainEventReader_i::serviceFunction()
{
