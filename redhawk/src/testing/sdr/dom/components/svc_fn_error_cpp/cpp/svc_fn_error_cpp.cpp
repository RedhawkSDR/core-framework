...
#include "svc_fn_error_cpp.h"

PREPARE_LOGGING(svc_fn_error_cpp_i)

svc_fn_error_cpp_i::svc_fn_error_cpp_i(const char *uuid, const char *label) :
    svc_fn_error_cpp_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

svc_fn_error_cpp_i::~svc_fn_error_cpp_i()
{
}

void svc_fn_error_cpp_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void svc_fn_error_cpp_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &svc_fn_error_cpp_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &svc_fn_error_cpp_i::<callback method>)
        in the constructor.
        ...
        //Add to svc_fn_error_cpp.cpp
        svc_fn_error_cpp_i::svc_fn_error_cpp_i(const char *uuid, const char *label) :
            svc_fn_error_cpp_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &svc_fn_error_cpp_i::scaleChanged);
            addPropertyListener(status, this, &svc_fn_error_cpp_i::statusChanged);
        }

        void svc_fn_error_cpp_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(svc_fn_error_cpp_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void svc_fn_error_cpp_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(svc_fn_error_cpp_i, "status changed");
        }
            
        //Add to svc_fn_error_cpp.h
...
************************************************************************************************/
int svc_fn_error_cpp_i::serviceFunction()
{
    throw std::runtime_error("test exception in serviceFunction()");
}
