...
from multiout_eos_base import *

class multiout_eos_i(multiout_eos_base):
    ...
    def process(self):
        ...
        Messages:
            ...
            self.port_input.registerMessage("my_msg", multiout_eos_i.MyMsg, self.msg_callback)
    ...
    *************************************************************'''
    ...
    def deviceSetTuning(self,request, fts, tuner_id):
        ...
        ************************************************************'''
        if self._usageState == CF.Device.ACTIVE:
            return False
        stream_id = "hello"
        new_connection = bulkio.connection_descriptor_struct()
        new_connection.connection_id = request.allocation_id;
        new_connection.port_name = self.port_dataShort_out.name
        new_connection.stream_id = stream_id

        self.connectionTable.append(new_connection)
        self.connectionTableChanged(self.connectionTable, self.connectionTable)
