...
# AUTO-GENERATED
#
# Source: fei_cap_py.spd.xml
from ossie.device import start_device
import logging

from fei_cap_py_base import *
from frontend import AllocationAlreadyExists
from ossie.utils import model
from omniORB import CORBA

class fei_cap_py_i(fei_cap_py_base):
    ...
    def allocate(self, capacities):
        ...
        for prop in capacities:
            if prop.id == "FRONTEND::tuner_allocation":
                tuner_alloc = prop.value._v
                for alloc in tuner_alloc:
                    if alloc.id == "FRONTEND::tuner_allocation::allocation_id":
                        requested_alloc = alloc.value._v
                        if requested_alloc:
                            if not (requested_alloc in self._delegatedAllocations):
                                allocation_id = requested_alloc
                            else:
                                raise AllocationAlreadyExists("ALLOCATION_ID ALREADY IN USE", requested_alloc)
                            alloc.value._v = allocation_id
    ...
    def deallocateCapacity (self, capacities):
        ...
        if allocation_id in self._delegatedAllocations:
            self._delegatedAllocations[allocation_id][0].device_ref.deallocateCapacity(capacities)
            self._delegatedAllocations.pop(allocation_id)
            self._usageState = self.updateUsageState()
            return

        fei_cap_py_base.deallocateCapacity(self, capacities)
    ...
    def process(self):
        ...
        Messages:
            ...
            self.port_input.registerMessage("my_msg", fei_cap_py_i.MyMsg, self.msg_callback)
