...
PREPARE_LOGGING(sri_i)

sri_i::sri_i(const char *uuid, const char *label) :
    sri_base(uuid, label)
{
    setPropertyChangeListener("mqd", this, &sri_i::mqdChanged);
}
...
void sri_i::mqdChanged (const std::string& id)
{
...
/***********************************************************************************************
    ...
    Properties:
        ...
        setPropertyChangeListener(<property name>, this, &sri_i::<callback method>)
        in the constructor.
        ...
        sri_i::sri_i(const char *uuid, const char *label) :
            sri_base(uuid, label)
        {
            setPropertyChangeListener("scaleValue", this, &sri_i::scaleChanged);
        }

        void sri_i::scaleChanged(const std::string& id){
...
************************************************************************************************/
int sri_i::serviceFunction()
{
    bulkio::InShortPort::dataTransfer *shortDT = dataShortIn->getPacket(0.0);
    if(shortDT){
        if(shortDT->sriChanged){
            dataShortOut->pushSRI(shortDT->SRI);
        }

        if(shortDT->inputQueueFlushed){
            shortDT->dataBuffer.clear();
            shortDT->dataBuffer.push_back(0);
        }
