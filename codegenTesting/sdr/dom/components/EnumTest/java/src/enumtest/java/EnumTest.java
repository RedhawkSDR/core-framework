 ...
 */
public class EnumTest extends EnumTest_base {
    /**
    ...
    public void runTest(int testid, CF.PropertiesHolder testValues) throws CF.UnknownProperties, CF.TestableObjectPackage.UnknownTest
    {
        switch (testid) {
        case 0:
            this.runEnumTest(testValues);
            break;
        default:
            throw new CF.TestableObjectPackage.UnknownTest();
        }
    }

    private CF.DataType[] toProperties(List<CF.DataType> props)
    {
        return props.toArray(new CF.DataType[props.size()]);
    }

    private CF.DataType createDataType(String id, float value)
    {
        org.omg.CORBA.Any any = orb.create_any();
        any.insert_float(value);
        return new CF.DataType(id, any);
    }

    private CF.DataType createDataType(String id, int value)
    {
        org.omg.CORBA.Any any = orb.create_any();
    ...
    private CF.DataType createDataType(String id, String value)
    {
        org.omg.CORBA.Any any = orb.create_any();
    ...
    private CF.DataType createDataType(String id, CF.DataType[] value)
    {
        org.omg.CORBA.Any any = orb.create_any();
        CF.PropertiesHelper.insert(any, value);
        return new CF.DataType(id, any);
    }

    private void runEnumTest(CF.PropertiesHolder testValues) throws CF.UnknownProperties
    {
        List<CF.DataType> unknown = new ArrayList<CF.DataType>();

        for (CF.DataType prop : testValues.value) {
            List<CF.DataType> value = new ArrayList<CF.DataType>();
            if (prop.id.equals("floatenum")) {
                value.add(createDataType("DEFAULT", enums.floatenum.DEFAULT));
                value.add(createDataType("OTHER", enums.floatenum.OTHER));
            } else if (prop.id.equals("stringenum")) {
                value.add(createDataType("START", enums.stringenum.START));
                value.add(createDataType("STOPPED", enums.stringenum.STOPPED));
            } else if (prop.id.equals("structprop")) {
                List<CF.DataType> number_enums = new ArrayList<CF.DataType>();
                number_enums.add(createDataType("ZERO", enums.structprop.number.ZERO));
                number_enums.add(createDataType("ONE", enums.structprop.number.ONE));
                number_enums.add(createDataType("TWO", enums.structprop.number.TWO));
                value.add(createDataType("structprop::number", toProperties(number_enums)));

                List<CF.DataType> alpha_enums = new ArrayList<CF.DataType>();
                alpha_enums.add(createDataType("ABC", enums.structprop.alpha.ABC));
                alpha_enums.add(createDataType("DEF", enums.structprop.alpha.DEF));
                value.add(createDataType("structprop::alpha", toProperties(alpha_enums)));
            } else if (prop.id.equals("structseq")) {
                List<CF.DataType> number_enums = new ArrayList<CF.DataType>();
                number_enums.add(createDataType("POSITIVE", enums.structseq_struct.number.POSITIVE));
                number_enums.add(createDataType("ZERO", enums.structseq_struct.number.ZERO));
                number_enums.add(createDataType("NEGATIVE", enums.structseq_struct.number.NEGATIVE));
                value.add(createDataType("structseq::number", toProperties(number_enums)));

                List<CF.DataType> text_enums = new ArrayList<CF.DataType>();
                text_enums.add(createDataType("HEADER", enums.structseq_struct.text.HEADER));
                text_enums.add(createDataType("BODY", enums.structseq_struct.text.BODY));
                text_enums.add(createDataType("FOOTER", enums.structseq_struct.text.FOOTER));
                value.add(createDataType("structseq::text", toProperties(text_enums)));
            } else {
                _logger.error("Unknown property " + prop.id);
                unknown.add(prop);
                continue;
            }
            prop.value = orb.create_any();
            CF.PropertiesHelper.insert(prop.value, toProperties(value));
        }

        if (unknown.size() > 0) {
            throw new CF.UnknownProperties(toProperties(unknown));
        }
    }
