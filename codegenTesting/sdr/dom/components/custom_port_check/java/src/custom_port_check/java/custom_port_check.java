package custom_port_check.java;
 ...
 *
 * Source: custom_port_check.spd.xml
 */
public class custom_port_check extends custom_port_check_base {
    /**
    ...
    protected int serviceFunction() {
    	DataType[] empty_prop_value = new DataType[0];
    	CF.PropertiesHolder empty_prop = new CF.PropertiesHolder();
    	empty_prop.value = empty_prop_value;
        this.inout_state.setValue("ok");
        try {
        	this.port_testableobject_out.runTest(1, empty_prop);
        } catch (PortCallError e) {
        	this.inout_state.setValue(e.getMessage());
        } catch (UnknownProperties | UnknownTest e) {
        	this.inout_state.setValue("unexpected exception");
        }

        String[] empty_string_seq = new String[0];
        this.retval_state.setValue("ok");
        try {
        	this.port_propertyemitter_out.registerPropertyListener(this.portServants.get("testableobject_out")._this_object(), empty_string_seq, (float)0.1);
        } catch (PortCallError e) {
        	this.retval_state.setValue(e.getMessage());
        } catch (UnknownProperties | InvalidObjectReference e) {
        	this.retval_state.setValue("unexpected exception");
        }

        byte[] out_data = new byte[0];
        this.in_state.setValue("ok");
        try {
        	this.port_file_out.write(out_data);
        } catch (PortCallError e) {
        	this.in_state.setValue(e.getMessage());
        } catch (IOException e) {
        	this.in_state.setValue("unexpected exception");
        }

        CF.OctetSequenceHolder data = new CF.OctetSequenceHolder();
        this.out_state.setValue("ok");
        try {
        	this.port_file_out.read(data, 10);
        } catch (PortCallError e) {
        	this.out_state.setValue(e.getMessage());
        } catch (IOException e) {
        	this.out_state.setValue("unexpected exception");
        }

        this.bad_connection.setValue("ok");
        try {
        	this.port_file_out.read(data, 10, "invalid_connectionid");
        } catch (PortCallError e) {
        	this.bad_connection.setValue(e.getMessage());
        } catch (IOException e) {
        	this.bad_connection.setValue("unexpected exception");
        }
