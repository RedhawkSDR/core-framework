...
void event_props_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void event_props_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &event_props_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &event_props_i::<callback method>)
        in the constructor.
        ...
        //Add to event_props.cpp
        event_props_i::event_props_i(const char *uuid, const char *label) :
            event_props_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &event_props_i::scaleChanged);
            addPropertyListener(status, this, &event_props_i::statusChanged);
        }

        void event_props_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void event_props_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            RH_DEBUG(this->_baseLog, "status changed");
        }
            
        //Add to event_props.h
