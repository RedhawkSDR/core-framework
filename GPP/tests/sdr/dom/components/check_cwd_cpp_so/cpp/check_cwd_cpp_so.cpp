...
PREPARE_LOGGING(check_cwd_cpp_so_i)

check_cwd_cpp_so_i::check_cwd_cpp_so_i(const char *uuid, const char *label) :
    check_cwd_cpp_so_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

check_cwd_cpp_so_i::~check_cwd_cpp_so_i()
{
}

void check_cwd_cpp_so_i::constructor()
{
    /***********************************************************************************
     This is the RH constructor. All properties are properly initialized before this function is called 
    ***********************************************************************************/
    char *tmp = getcwd(NULL, 200);
    if (tmp != NULL) {
        this->cwd = std::string(tmp);
        free(tmp);
    }
}
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void check_cwd_cpp_so_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &check_cwd_cpp_so_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &check_cwd_cpp_so_i::<callback method>)
        in the constructor.
        ...
        //Add to check_cwd_cpp_so.cpp
        check_cwd_cpp_so_i::check_cwd_cpp_so_i(const char *uuid, const char *label) :
            check_cwd_cpp_so_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &check_cwd_cpp_so_i::scaleChanged);
            addPropertyListener(status, this, &check_cwd_cpp_so_i::statusChanged);
        }

        void check_cwd_cpp_so_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(check_cwd_cpp_so_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void check_cwd_cpp_so_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(check_cwd_cpp_so_i, "status changed");
        }
            
        //Add to check_cwd_cpp_so.h
...
************************************************************************************************/
int check_cwd_cpp_so_i::serviceFunction()
{
    LOG_DEBUG(check_cwd_cpp_so_i, "serviceFunction() example log message");
