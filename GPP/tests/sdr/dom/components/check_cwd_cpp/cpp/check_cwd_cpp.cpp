...
PREPARE_LOGGING(check_cwd_cpp_i)

check_cwd_cpp_i::check_cwd_cpp_i(const char *uuid, const char *label) :
    check_cwd_cpp_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

check_cwd_cpp_i::~check_cwd_cpp_i()
{
}

void check_cwd_cpp_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void check_cwd_cpp_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &check_cwd_cpp_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &check_cwd_cpp_i::<callback method>)
        in the constructor.
        ...
        //Add to check_cwd_cpp.cpp
        check_cwd_cpp_i::check_cwd_cpp_i(const char *uuid, const char *label) :
            check_cwd_cpp_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &check_cwd_cpp_i::scaleChanged);
            addPropertyListener(status, this, &check_cwd_cpp_i::statusChanged);
        }

        void check_cwd_cpp_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(check_cwd_cpp_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void check_cwd_cpp_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(check_cwd_cpp_i, "status changed");
        }
            
        //Add to check_cwd_cpp.h
...
************************************************************************************************/
int check_cwd_cpp_i::serviceFunction()
{
    LOG_DEBUG(check_cwd_cpp_i, "serviceFunction() example log message");
