...
class ImplementationSettings(object):
    def __init__(self, name=None, outputDir=None, template=None, properties={}, crcs={}, children=[]):
        self.name = name
...
def _importWavedevImplSettings(node, children):
...
def importWavedevSettings(filename):
    ...
    for node in dom.getElementsByTagName('implSettings'):
        ...
        for _node in node.childNodes:
            if _node.nodeName == 'child':
                children.append(copy.deepcopy(_node)) # the copy is needed because subsequent parsing in the functions does not include the parsed structure
        implId = node.getAttribute('key')
        settings[implId] = _importWavedevImplSettings(node.getElementsByTagName('value')[0], children)
    dom.unlink()
    return settings
