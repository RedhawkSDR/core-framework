...
class FrontendTunerDevice(Device):
    ...
    def deallocateCapacity(self, properties):
        ...
        Output:
            None
        """
        self._device_log.debug("deallocateCapacity(%s)", properties)
        # Validate
        ...
        self._device_log.debug("deallocateCapacity() -->")
    ...
    def allocateCapacity(self, properties):
        self._device_log.debug("allocateCapacity(%s)", properties)
    ...
    def _allocate_frontend_tuner_allocation(self, frontend_tuner_allocation, scanner_prop = None):
        ...
        try:
            ...
            if not frontend_tuner_allocation.allocation_id:
                self._device_log.info("allocate_frontend_tuner_allocation: MISSING ALLOCATION_ID")
            ...
            if  self.getTunerMapping(frontend_tuner_allocation.allocation_id) >= 0:
                self._device_log.info("allocate_frontend_tuner_allocation: ALLOCATION_ID "+frontend_tuner_allocation.allocation_id+" ALREADY IN USE")
            ...
            try:
                ...
                for tuner_id in range(len(self.tuner_allocation_ids)):
                    if self.frontend_tuner_status[tuner_id].tuner_type != frontend_tuner_allocation.tuner_type:
                        self._device_log.debug("allocate_frontend_tuner_allocation: Requested tuner type '" + str(frontend_tuner_allocation.tuner_type) + "' does not match tuner[" + str(tuner_id) + "].tuner_type ("+str(self.frontend_tuner_status[tuner_id].tuner_type)+")")
                        continue
                    ...
                    if len(frontend_tuner_allocation.group_id) > 0 and frontend_tuner_allocation.group_id != self.frontend_tuner_status[tuner_id].group_id:
                        self._device_log.debug("allocate_frontend_tuner_allocation: Requested group_id '"+str(frontend_tuner_allocation.group_id)+"' does not match tuner[" + str(tuner_id) + "].group_id ("+str(self.frontend_tuner_status[tuner_id].group_id)+")")
                        continue
                    ...
                    if len(frontend_tuner_allocation.rf_flow_id) > 0 and \
                       ...
                       frontend_tuner_allocation.tuner_type != "CHANNELIZER":
                        self._device_log.debug("allocate_frontend_tuner_allocation: Requested rf_flow_id '"+str(frontend_tuner_allocation.rf_flow_id)+"' does not match tuner[" +str(tuner_id)+ "].rf_flow_id ("+str(self.frontend_tuner_status[tuner_id].rf_flow_id)+")")
                        continue
                    ...
                    if frontend_tuner_allocation.device_control:
                        ...
                        if self.supports_scan and self._has_scanner:
                            if len(self.tuner_allocation_ids[tuner_id].control_allocation_id)>0 or not self.deviceSetTuningScan(frontend_tuner_allocation, scanner_prop, self.frontend_tuner_status[tuner_id], tuner_id):
                                ...
                                self._device_log.debug("allocate_frontend_tuner_allocation: Tuner["+str(tuner_id)+"] is either not available or didn't succeed while setting tuning ")
                                continue
                        else:
                        ...
                        if len(self.tuner_allocation_ids[tuner_id].control_allocation_id) == 0 or not self.listenerRequestValidation(frontend_tuner_allocation, tuner_id):
                            ...
                            self._device_log.debug("allocate_frontend_tuner_allocation: Tuner["+str(tuner_id)+"] is either not available or can not support listener request ")
                            continue
                    ...
                    self._device_log.debug(" allocate_frontend_tuner_allocation - SR requested: " + str(frontend_tuner_allocation.sample_rate) + "  SR got: " +str(self.frontend_tuner_status[tuner_id].sample_rate))
                    if  floatingPointCompare(frontend_tuner_allocation.sample_rate,0)!=0 and  \
                        ...
                        self._device_log.info(eout)
                        raise allocationException(eout)

                    self._device_log.debug(" allocate_frontend_tuner_allocation - BW requested: " + str(frontend_tuner_allocation.bandwidth) + "  BW got: " +str(self.frontend_tuner_status[tuner_id].bandwidth))
                ...
                self._device_log.info(e.message)
                raise
            except Exception as e:
                self._device_log.exception(e.message)
            ...
            self._device_log.exception('The following error occurred on allocation:',e)
            #self.deallocateCapacity([frontend_tuner_allocation.getProp()])
            raise e
    ...
    def deallocate_frontend_tuner_allocation(self, frontend_tuner_allocation):
        ...
        tuner_id = self.getTunerMapping(frontend_tuner_allocation.allocation_id)
        if tuner_id < 0:
            self._device_log.debug("deallocate_frontend_tuner_allocation: ALLOCATION_ID NOT FOUND: [" + str(frontend_tuner_allocation.allocation_id) + "]")
    ...
    def _allocate_frontend_listener_allocation(self, frontend_listener_allocation):
        try:
            ...
            if not frontend_listener_allocation.existing_allocation_id:
                self._device_log.info("allocate_frontend_listener_allocation: MISSING EXISTING ALLOCATION ID")
            ...
            if not frontend_listener_allocation.listener_allocation_id:
                self._device_log.info("allocate_frontend_listener_allocation: MISSING LISTENER ALLOCATION ID")
            ...
            if self.getTunerMapping(frontend_listener_allocation.listener_allocation_id) >= 0:
                self._device_log.info("allocate_frontend_listener_allocation: LISTENER ALLOCATION ID ALREADY IN USE: [" + str(frontend_listener_allocation.listener_allocation_id + "]"))
            ...
            if tuner_id < 0:
                self._device_log.debug("allocate_frontend_listener_allocation: UNKNOWN CONTROL ALLOCATION ID: [" + str(frontend_listener_allocation.existing_allocation_id)+"]")
            ...
            self._device_log.info('The following error occurred on allocation:',e)
            raise e
    ...
    def deallocate_frontend_listener_allocation(self, frontend_listener_allocation):
        ...
        if tuner_id < 0:
            self._device_log.debug("ALLOCATION_ID NOT FOUND: [" + str(frontend_listener_allocation.listener_allocation_id) + "]")
        ...
        if self.tuner_allocation_ids[tuner_id].control_allocation_id == frontend_listener_allocation.listener_allocation_id:
            self._device_log.debug("Controlling allocation id cannot be used as a listener id in a deallocation: [" + str(frontend_listener_allocation.listener_allocation_id) + "]")
            retval_struct = [CF.DataType(id='FRONTEND::listener_allocation::existing_allocation_id',value=any.to_any(frontend_listener_allocation.existing_allocation_id)),CF.DataType(id='FRONTEND::listener_allocation::listener_allocation_id',value=any.to_any(frontend_listener_allocation.existing_allocation_id))]
            retval = CF.DataType(id='FRONTEND::listener_allocation',value=any.to_any(retval_struct))
    ...
    def listenerRequestValidation(self, request, tuner_id):
        self._device_log.trace("listenerRequestValidation(): request " + str(request) + " ,tuner_id " + str(tuner_id))
        ...
        if floatingPointCompare((request.center_frequency-(request.bandwidth*0.5)),(self.frontend_tuner_status[tuner_id].center_frequency-(self.frontend_tuner_status[tuner_id].bandwidth*0.5))) < 0:
            self._device_log.trace("listenerRequestValidation(): FAILED LOWER END TEST")
            return False

        # ensure upper end of requested band fits
        if floatingPointCompare((request.center_frequency + (request.bandwidth*0.5)),(self.frontend_tuner_status[tuner_id].center_frequency + (self.frontend_tuner_status[tuner_id].bandwidth*0.5))) > 0:
            self._device_log.trace("listenerRequestValidation(): FAILED UPPER END TEST")
            return False
    ...
    def removeTunerMappingByAllocationId(self, allocation_id):
        self._device_log.trace("removeTunerMapping(allocation_id) allocation_id " + str(allocation_id))
        self.allocation_id_mapping_lock.acquire()
        try:
