...
template < typename PT, typename IP >
  class Test_Manager_Base : public CppUnit::TestFixture
{
    typedef custom_transport::CustomTransportFactory FACTORY;
    typedef bulkio::InputManager< PT >               IM;
    typedef bulkio::InputTransport< PT >             TRANSPORT_BASE;
    typedef custom_transport::CustomInputTransport   TRANSPORT;
    ...
    CPPUNIT_TEST( test_create_transport_good );
...
public:
    ...
    void test_create_transport_good( );
...
#define CHECK_PROP_NOT_EXISTS(pid)   \
    { \
      ...
      bool val=props.contains(prop_id); \
      bool exp=true; \
      CPPUNIT_ASSERT_ASSERTION_FAIL_MESSAGE( "transport properties should not "#pid , CPPUNIT_ASSERT_EQUAL( exp,  val ) ); \
   }



template< typename PT, typename IP >
void Test_Manager_Base< PT, IP >::setUp()
{
    ...
    transport=0;
    custom_transport=0;
    logger=rh_logger::Logger::getLogger("InManagerTest");
    setUp_bulkio_test();
}

template< typename PT, typename IP >
void Test_Manager_Base< PT, IP >::setUp_post()
{
    factory = new FACTORY();
    port= new IP("inport");
    // manager=dynamic_cast< custom_transport::CustomInputManager * >(factory->createInputManager( port ));
    manager=factory->createInputManager( port );
    transport=0;
    custom_transport=0;
}
...
template< typename PT, typename IP >
void Test_Manager_Base< PT, IP >::tearDown()
{
...
template< typename PT, typename IP >
void Test_Manager_Base< PT, IP >::test_transport_type()
{
    setUp_bulkio_test();
  setUp_post();
   CPPUNIT_ASSERT_MESSAGE( "input manager create failed ", manager != 0 );
...
template< typename PT, typename IP >
void Test_Manager_Base< PT, IP >::test_transport_properties()
{
  setUp_bulkio_test();
  setUp_post();
  CPPUNIT_ASSERT_MESSAGE( "input manager create failed ", manager != 0 );
...
template< typename PT, typename IP >
void Test_Manager_Base< PT, IP >::test_transport_properties_disable()
{
    ...
    bool val=props.contains("custom::disable");
    bool exp=true;
    CPPUNIT_ASSERT_ASSERTION_PASS_MESSAGE( "Transport properites missing custom::disable", CPPUNIT_ASSERT_EQUAL( exp,val ));

    int disable=props["custom::disable"].toLong();
    int exp_i=1;
    CPPUNIT_ASSERT_ASSERTION_PASS_MESSAGE( "Transport properites, custom::disable reporting incorrect value", CPPUNIT_ASSERT_EQUAL( exp_i,disable));
}


template< typename PT, typename IP >
void Test_Manager_Base< PT, IP >::test_create_transport_disable_env()
{
    setUp_disable_custom();
    setUp_post();
    CPPUNIT_ASSERT_MESSAGE( "input manager create failed ", manager != 0 );

    std::string transportId("custom-transport-id");
    redhawk::PropertyMap props;

    transport=manager->createInputTransport( transportId, props );
    TRANSPORT_BASE *exp_t=0;
    CPPUNIT_ASSERT_ASSERTION_PASS_MESSAGE( "create transport did not fail.", CPPUNIT_ASSERT( exp_t == transport ) );
}

template< typename PT, typename IP >
void Test_Manager_Base< PT, IP >::test_create_transport_good()
{
    setUp_bulkio_test();
    setUp_post();
    CPPUNIT_ASSERT_MESSAGE( "input manager create failed ", manager != 0 );
    ...
    // check that data end point was created..
    props=redhawk::PropertyMap::cast(custom_transport->getNegotiationProperties());
    CHECK_PROP_EXISTS("bulkio::custom::data::address");
    CHECK_PROP_EXISTS("bulkio::custom::data::port");
    CHECK_PROP_EXISTS("bulkio::custom::data::protocol");
    CHECK_PROP_EXISTS("bulkio::custom::data::requestSize");

}


template< typename PT, typename IP >
void Test_Manager_Base< PT, IP >::test_get_negotiation_props_bad_transport()
{
    setUp_post();
    // test bad parameter passing
...
template< typename PT, typename IP >
void Test_Manager_Base< PT, IP >::test_get_negotiation_props()
{
    setUp_bulkio_test();
    setUp_post();
    CPPUNIT_ASSERT_MESSAGE( "input manager create failed ", manager != 0 );
    ...
    props=manager->getNegotiationProperties(transport);
    CHECK_PROP_EXISTS("bulkio::custom::data::address");
    CHECK_PROP_EXISTS("bulkio::custom::data::port");
    CHECK_PROP_EXISTS("bulkio::custom::data::protocol");
    CHECK_PROP_EXISTS("bulkio::custom::data::requestSize");
...
#define CREATE_TEST(x,y,pp)						\
  class Custom_InManager_##x##_##pp##_Test : public Test_Manager_Base< BULKIO::data##x , bulkio::In##y##Port  > \
    {                                                                   \
      typedef Test_Manager_Base< BULKIO::data##x , bulkio::In##y##Port > TestBase;	\
        CPPUNIT_TEST_SUB_SUITE(Custom_InManager_##x##_##pp##_Test, TestBase);               \
        CPPUNIT_TEST_SUITE_END();                                       \
  ...
  CPPUNIT_TEST_SUITE_REGISTRATION(Custom_InManager_##x##_##pp##_Test);
