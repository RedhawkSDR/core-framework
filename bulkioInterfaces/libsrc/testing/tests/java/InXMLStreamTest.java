...
public class InXMLStreamTest {

    /**
     * Input port being tested
     */
    protected InXMLPort port;
    ...
    public void testGetCurrentStreamEmptyPacket()
    {
        ...
        InXMLStream stream = port.getCurrentStream(bulkio.Const.NON_BLOCKING);
    ...
    public void testGetCurrentStreamEmptyEos()
    {
        ...
        InXMLStream stream = port.getStream("empty_eos");
        Assert.assertNotNull(stream);
        XMLDataBlock block = stream.read();
        Assert.assertNotNull(block);
        ...
        // removed it
        InXMLStream next_stream = port.getCurrentStream(bulkio.Const.NON_BLOCKING);
    ...
    public void testTryread()
    {
        ...
        InXMLStream stream = port.getStream(stream_id);
        Assert.assertNotNull(stream);
        XMLDataBlock block = stream.tryread();
        Assert.assertEquals(1024, block.buffer().length());
        block = stream.read();
        Assert.assertNull(block);
    }
    ...
    public void testRepeatStreamIds()
    {
        ...
        for (int i=0; i<number_streams; i++) {
            ...
            InXMLStream inputStream = port.getCurrentStream(bulkio.Const.NON_BLOCKING);
            Assert.assertNotNull(inputStream);
            received_streams += 1;
            for (int j=0; j<number_packets; j++) {
                XMLDataBlock block = inputStream.read();
                Assert.assertNotNull(block);
                received_packets += 1;
            }
