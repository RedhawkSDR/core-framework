 ...
 */
/*
 * WARNING: This file is generated from NumericInPortTestImpl.java.template.
...
public class NumericInPortTestImpl<E extends BULKIO.updateSRIOperations & BULKIO.ProvidesPortStatisticsProviderOperations,A> {
    ...
    public NumericInPortTestImpl(TestHelper<E,A> helper)
    {
        this.helper = helper;
    }
    ...
    public void testQueueFlushScenarios()
    {
        ...
        BULKIO.PrecisionUTCTime ts = bulkio.time.utils.create(100.0, 0.0, (short)1);
        ...
        helper.pushTestPacket(port, 1, ts, false, stream_A.streamID);
        ...
        helper.pushTestPacket(port, 2, bulkio.time.utils.increment(ts,0.1), false, stream_B.streamID);
        ...
        helper.pushTestPacket(port, 3, bulkio.time.utils.increment(ts,0.2), false, stream_C.streamID);
        ...
        // Test case 1
        helper.pushTestPacket(port, 4, bulkio.time.utils.increment(ts,1), false, stream_B.streamID);
        helper.pushTestPacket(port, 5, bulkio.time.utils.increment(ts,2), false, stream_C.streamID);
        helper.pushTestPacket(port, 6, bulkio.time.utils.increment(ts,3), false, stream_B.streamID);
        helper.pushTestPacket(port, 7, bulkio.time.utils.increment(ts,4), false, stream_C.streamID);
        port.setMaxQueueDepth(4);
        // flush
        helper.pushTestPacket(port, 8, bulkio.time.utils.increment(ts, 5), false, stream_A.streamID);
        packet = port.getPacket(bulkio.Const.NON_BLOCKING);
        Assert.assertNotNull(packet);
        Assert.assertEquals(stream_B.streamID, packet.streamID);
        Assert.assertTrue(packet.inputQueueFlushed);
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,3)), 0);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,4)), 0);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,5)), 0);
        ...
        helper.pushTestPacket(port, 7, bulkio.time.utils.increment(ts, 4), false, stream_A.streamID);
        helper.pushTestPacket(port, 8, bulkio.time.utils.increment(ts, 5), false, stream_B.streamID);
        helper.pushTestPacket(port, 9, bulkio.time.utils.increment(ts, 6), false, stream_C.streamID);
        helper.pushTestPacket(port, 10, bulkio.time.utils.increment(ts, 7), false, stream_A.streamID);
        helper.pushTestPacket(port, 11, bulkio.time.utils.increment(ts, 8), false, stream_B.streamID);
        helper.pushTestPacket(port, 12, bulkio.time.utils.increment(ts, 9), false, stream_C.streamID);
        // flush
        helper.pushTestPacket(port, 13, bulkio.time.utils.increment(ts, 10), false, stream_A.streamID);
        packet = port.getPacket(bulkio.Const.NON_BLOCKING);
        Assert.assertNotNull(packet);
        Assert.assertEquals(stream_B.streamID, packet.streamID);
        Assert.assertTrue(packet.inputQueueFlushed);
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,8)), 0);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,9)), 0);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,10)), 0);
        ...
        // Test case 3
        helper.pushTestPacket(port, 11, bulkio.time.utils.increment(ts, 11), false, stream_B.streamID);
        helper.pushTestPacket(port, 12, bulkio.time.utils.increment(ts, 12), false, stream_B.streamID);
        helper.pushTestPacket(port, 0, bulkio.time.utils.increment(ts, 13), true, stream_B.streamID);
        helper.pushTestPacket(port, 13, bulkio.time.utils.increment(ts, 14), false, stream_C.streamID);
        helper.pushTestPacket(port, 14, bulkio.time.utils.increment(ts, 15), false, stream_C.streamID);
        // flush
        helper.pushTestPacket(port, 15, bulkio.time.utils.increment(ts, 16), false, stream_A.streamID);
        packet = port.getPacket(bulkio.Const.NON_BLOCKING);
        Assert.assertNotNull(packet);
        Assert.assertEquals(stream_B.streamID, packet.streamID);
        Assert.assertTrue(packet.inputQueueFlushed);
        Assert.assertTrue(packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,12)), 0);
        Assert.assertEquals(12, helper.dataLength(packet.dataBuffer));
        packet = port.getPacket(bulkio.Const.NON_BLOCKING);
        Assert.assertNotNull(packet);
        Assert.assertEquals(stream_C.streamID, packet.streamID);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,15)), 0);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,16)), 0);
        ...
        stream_A.blocking = false;
        corbaPort.pushSRI(stream_A);
        helper.pushTestPacket(port, 14, bulkio.time.utils.increment(ts, 17), false, stream_A.streamID);

        // Push 1 packet for stream_B
        stream_B = bulkio.sri.utils.create("stream_B");
        stream_B.blocking = false;
        corbaPort.pushSRI(stream_B);
        helper.pushTestPacket(port, 15, bulkio.time.utils.increment(ts, 18), false, stream_B.streamID);

        // Push 1 packet for stream_C
        stream_C = bulkio.sri.utils.create("stream_C");
        stream_C.blocking = false;
        corbaPort.pushSRI(stream_C);
        helper.pushTestPacket(port, 16, bulkio.time.utils.increment(ts, 19), false, stream_C.streamID);
        ...
        // Test case 4
        port.setMaxQueueDepth(3);
        helper.pushTestPacket(port, 0, bulkio.time.utils.increment(ts, 20), true, stream_B.streamID);
        helper.pushTestPacket(port, 17, bulkio.time.utils.increment(ts, 21), false, stream_C.streamID);
        helper.pushTestPacket(port, 18, bulkio.time.utils.increment(ts, 22), false, stream_C.streamID);
        // flush
        helper.pushTestPacket(port, 19, bulkio.time.utils.increment(ts, 23), false, stream_A.streamID);
        packet = port.getPacket(bulkio.Const.NON_BLOCKING);
        Assert.assertNotNull(packet);
        Assert.assertEquals(stream_B.streamID, packet.streamID);
        Assert.assertTrue(!packet.inputQueueFlushed);
        Assert.assertTrue(packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,20)), 0);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,22)), 0);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,23)), 0);
        ...
        stream_A.blocking = false;
        corbaPort.pushSRI(stream_A);
        helper.pushTestPacket(port, 19, bulkio.time.utils.increment(ts, 24), false, stream_A.streamID);

        // Push 1 packet for stream_B
        stream_B = bulkio.sri.utils.create("stream_B");
        stream_B.blocking = false;
        corbaPort.pushSRI(stream_B);
        helper.pushTestPacket(port, 20, bulkio.time.utils.increment(ts, 25), false, stream_B.streamID);

        // Push 1 packet for stream_C
        stream_C = bulkio.sri.utils.create("stream_C");
        stream_C.blocking = false;
        corbaPort.pushSRI(stream_C);
        helper.pushTestPacket(port, 21, bulkio.time.utils.increment(ts, 26), false, stream_C.streamID);
        ...
        // Test case 5
        port.setMaxQueueDepth(3);
        helper.pushTestPacket(port, 22, bulkio.time.utils.increment(ts, 27), false, stream_A.streamID);
        helper.pushTestPacket(port, 0, bulkio.time.utils.increment(ts, 28), true, stream_A.streamID);
        corbaPort.pushSRI(stream_A);
        helper.pushTestPacket(port, 23, bulkio.time.utils.increment(ts, 29), false, stream_A.streamID);
        // flush
        helper.pushTestPacket(port, 24, bulkio.time.utils.increment(ts, 30), false, stream_A.streamID);
        packet = port.getPacket(bulkio.Const.NON_BLOCKING);
        Assert.assertNotNull(packet);
        Assert.assertEquals(stream_A.streamID, packet.streamID);
        Assert.assertTrue(!packet.inputQueueFlushed);
        Assert.assertTrue(packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,27)), 0);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,30)), 0);
        ...
        // Test case 5a
        port.setMaxQueueDepth(4);
        helper.pushTestPacket(port, 21, bulkio.time.utils.increment(ts, 31), false, stream_A.streamID);
        helper.pushTestPacket(port, 22, bulkio.time.utils.increment(ts, 32), false, stream_A.streamID);
        helper.pushTestPacket(port, 0, bulkio.time.utils.increment(ts, 33), true, stream_A.streamID);
        corbaPort.pushSRI(stream_A);
        helper.pushTestPacket(port, 23, bulkio.time.utils.increment(ts, 34), false, stream_A.streamID);
        // flush
        helper.pushTestPacket(port, 24, bulkio.time.utils.increment(ts, 35), false, stream_A.streamID);
        packet = port.getPacket(bulkio.Const.NON_BLOCKING);
        Assert.assertNotNull(packet);
        Assert.assertEquals(stream_A.streamID, packet.streamID);
        Assert.assertTrue(packet.inputQueueFlushed);
        Assert.assertTrue(packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,32)), 0);
        Assert.assertEquals(22, helper.dataLength(packet.dataBuffer));
        packet = port.getPacket(bulkio.Const.NON_BLOCKING);
        Assert.assertNotNull(packet);
        Assert.assertEquals(stream_A.streamID, packet.streamID);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,35)), 0);
        Assert.assertEquals(24, helper.dataLength(packet.dataBuffer));
        ...
        // Test case 6
        port.setMaxQueueDepth(3);
        helper.pushTestPacket(port, 0, bulkio.time.utils.increment(ts, 36), true, stream_A.streamID);
        helper.pushTestPacket(port, 25, bulkio.time.utils.increment(ts, 37), false, stream_B.streamID);
        helper.pushTestPacket(port, 26, bulkio.time.utils.increment(ts, 38), false, stream_B.streamID);
        // flush
        ...
        corbaPort.pushSRI(stream_A);
        helper.pushTestPacket(port, 27, bulkio.time.utils.increment(ts, 39), false, stream_A.streamID);
        packet = port.getPacket(bulkio.Const.NON_BLOCKING);
        Assert.assertNotNull(packet);
        Assert.assertEquals(stream_A.streamID, packet.streamID);
        Assert.assertTrue(!packet.inputQueueFlushed);
        Assert.assertTrue(packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,36)), 0);
        Assert.assertEquals(0, helper.dataLength(packet.dataBuffer));
        packet = port.getPacket(bulkio.Const.NON_BLOCKING);
        Assert.assertNotNull(packet);
        Assert.assertEquals(stream_B.streamID, packet.streamID);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,38)), 0);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,39)), 0);
        ...
        corbaPort.pushSRI(stream_A);
        helper.pushTestPacket(port, 28, bulkio.time.utils.increment(ts, 40), false, stream_A.streamID);
        packet = port.getPacket(bulkio.Const.NON_BLOCKING);
        Assert.assertNotNull(packet);
        Assert.assertEquals(stream_A.streamID, packet.streamID);
        Assert.assertTrue(!packet.inputQueueFlushed);
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,40)), 0);
        ...
        corbaPort.pushSRI(stream_A);
        helper.pushTestPacket(port, 28, bulkio.time.utils.increment(ts, 41), false, stream_A.streamID);
        helper.pushTestPacket(port, 29, bulkio.time.utils.increment(ts, 42), false, stream_C.streamID);
        helper.pushTestPacket(port, 30, bulkio.time.utils.increment(ts, 43), false, stream_A.streamID);
        helper.pushTestPacket(port, 31, bulkio.time.utils.increment(ts, 44), false, stream_C.streamID);
        // flush
        helper.pushTestPacket(port, 32, bulkio.time.utils.increment(ts, 45), false, stream_B.streamID);
        packet = port.getPacket(bulkio.Const.NON_BLOCKING);
        Assert.assertNotNull(packet);
        Assert.assertEquals(stream_A.streamID, packet.streamID);
        Assert.assertTrue(packet.inputQueueFlushed);
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,43)), 0);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,44)), 0);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,45)), 0);
        ...
        corbaPort.pushSRI(stream_A);
        helper.pushTestPacket(port, 28, bulkio.time.utils.increment(ts, 46), false, stream_A.streamID);
        helper.pushTestPacket(port, 29, bulkio.time.utils.increment(ts, 47), false, stream_C.streamID);
        helper.pushTestPacket(port, 30, bulkio.time.utils.increment(ts, 48), false, stream_A.streamID);
        // flush
        helper.pushTestPacket(port, 32, bulkio.time.utils.increment(ts, 49), false, stream_B.streamID);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,47)), 0);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,48)), 0);
        Assert.assertEquals(30, helper.dataLength(packet.dataBuffer));
        packet = port.getPacket(bulkio.Const.NON_BLOCKING);
        Assert.assertNotNull(packet);
        Assert.assertEquals(stream_B.streamID, packet.streamID);
        ...
        Assert.assertTrue(!packet.EOS);
        Assert.assertTrue(!packet.sriChanged);
        Assert.assertEquals(bulkio.time.utils.compare(packet.T, bulkio.time.utils.increment(ts,49)), 0);
        Assert.assertEquals(32, helper.dataLength(packet.dataBuffer));
