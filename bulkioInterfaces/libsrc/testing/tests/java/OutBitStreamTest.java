...
public class OutBitStreamTest {

    /**
     * Input port being tested
     */
    protected OutBitPort port;
    protected Stub<BULKIO.BitSequence> stub;
    ...
    public OutBitStreamTest()
    {
        this.helper = new BitTestHelper();
    }

    protected Stub<BULKIO.BitSequence> _createStub() throws org.omg.CORBA.UserException
    {
        Stub<BULKIO.BitSequence> new_stub = helper.createStub();
    ...
    protected void _writeSinglePacket(OutBitStream stream, int size)
    {
        BULKIO.PrecisionUTCTime t = bulkio.time.utils.now();
        this._writeSinglePacket(stream, size, t);
    }

    protected void _writeSinglePacket(OutBitStream stream, int size, BULKIO.PrecisionUTCTime time)
    {
        bitbuffer buffer = new bitbuffer(size);
        stream.write(buffer, time);
    }

    protected void _writeTimestampsImpl(OutBitStream stream, boolean complexData)
    {
        // Generate a ramp using the scalar type; if the data needs to be pushed as
        // complex, it will be reintepreted there
        bitbuffer scalars = new bitbuffer(200);
        ...
        for (int ii = 0; ii < stub.packets.size(); ++ii) {
            ...
            scalars_received += stub.packets.get(ii).data.bits;
        }
    ...
    {
        // Create a new stream
        OutBitStream good_stream = this.port.createStream("test_operators");
        Assert.assertNotNull(good_stream);

        // Get another handle to the same stream, should be equal
        OutBitStream same_stream = port.getStream("test_operators");
        Assert.assertEquals(same_stream, good_stream);

        // Create a new stream, should not be equal
        OutBitStream other_stream = port.createStream("test_operators_2");
    ...
    {
        OutBitStream stream = port.createStream("test_basic_write");
        ...
        Assert.assertEquals(256, stub.packets.get(0).data.bits);
    ...
    public void testSriFields()
    {
        ...
        OutBitStream stream = port.createStream(sri);
    ...
    public void testSriNoChange()
    {
        ...
        OutBitStream stream = port.createStream("test_sri_no_change");
    ...
    public void testSriUpdate()
    {
        ...
        // write
        OutBitStream stream = port.createStream("test_sri_update");
    ...
    {
        OutBitStream stream = port.createStream("test_keywords");
    ...
    {
        OutBitStream stream = port.createStream("close_eos");
    ...
    public void testBufferedWrite()
    {
        ...
        OutBitStream stream = port.createStream("test_buffered_write");
        ...
        bitbuffer buffer = new bitbuffer(48);
        ...
        stream.write(buffer, bulkio.time.utils.now());
        Assert.assertTrue(stub.packets.size() == 1);
        Assert.assertEquals(stream.bufferSize(), stub.packets.get(stub.packets.size()-1).data.bits);
        ...
        stream.flush();
        Assert.assertTrue(stub.packets.size() == 2);
        Assert.assertEquals(64, stub.packets.get(stub.packets.size()-1).data.bits);
    ...
    {
        // Turn on buffering
        OutBitStream stream = port.createStream("test_skip_buffer");
        stream.setBufferSize(100);

        // With an empty queue, large write should go right through
        bitbuffer buffer = new bitbuffer(256);
        stream.write(buffer, bulkio.time.utils.now());
        Assert.assertTrue(stub.packets.size() == 1);
        Assert.assertEquals(buffer.length, stub.packets.get(stub.packets.size()-1).data.bits);
    ...
    {
        // Turn on buffering
        OutBitStream stream = port.createStream("test_flush");
    ...
    {
        OutBitStream stream = port.createStream("test_flush_close");
        stream.setBufferSize(64);
    ...
    public void testFlushOnSriChange()
    {
        ...
        OutBitStream stream = port.createStream("test_flush_sri");
        stream.setBufferSize(64);
        stream.xdelta(0.125);
    ...
    {
        OutBitStream stream = port.createStream("test_flush_buffer_size");
        stream.setBufferSize(64);
    ...
    {
        OutBitStream stream = port.createStream("compound_push");
        // Generate a ramp using the scalar type; if the data needs to be pushed as
        // complex, it will be reintepreted there
        ...
        int push_size = 801;
        bitbuffer scalars = new bitbuffer(push_size);
        for (int ii = 0; ii < scalars.length; ++ii) {
            if (ii%2 == 0) {
                scalars.set(ii);
            }
    ...
    {
        OutBitStream stream = port.createStream("write_timestamps_real");
