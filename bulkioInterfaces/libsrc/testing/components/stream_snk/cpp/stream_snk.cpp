...
PREPARE_LOGGING(stream_snk_i)

stream_snk_i::stream_snk_i(const char *uuid, const char *label) :
    stream_snk_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

stream_snk_i::~stream_snk_i()
{
}

void stream_snk_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void stream_snk_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &stream_snk_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &stream_snk_i::<callback method>)
        in the constructor.
        ...
        //Add to stream_snk.cpp
        stream_snk_i::stream_snk_i(const char *uuid, const char *label) :
            stream_snk_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &stream_snk_i::scaleChanged);
            addPropertyListener(status, this, &stream_snk_i::statusChanged);
        }

        void stream_snk_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void stream_snk_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
...
************************************************************************************************/
int stream_snk_i::serviceFunction()
{
    RH_DEBUG(this->_baseLog, "serviceFunction() example log message");
    bulkio::InFloatStream inputStream = dataFloat_in->getCurrentStream();
    if (!inputStream) { // No streams are available
        return NOOP;
    }
    int block_num = 0;
    bulkio::FloatDataBlock block = inputStream.read();
    while (block) { // No data available
    	block_num++;
        // Propagate end-of-stream
