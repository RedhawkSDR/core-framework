...
PREPARE_LOGGING(StreamWriteCheck_i)

StreamWriteCheck_i::StreamWriteCheck_i(const char *uuid, const char *label) :
    StreamWriteCheck_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

StreamWriteCheck_i::~StreamWriteCheck_i()
{
}

void StreamWriteCheck_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void StreamWriteCheck_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &StreamWriteCheck_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &StreamWriteCheck_i::<callback method>)
        in the constructor.
        ...
        //Add to StreamWriteCheck.cpp
        StreamWriteCheck_i::StreamWriteCheck_i(const char *uuid, const char *label) :
            StreamWriteCheck_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &StreamWriteCheck_i::scaleChanged);
            addPropertyListener(status, this, &StreamWriteCheck_i::statusChanged);
        }

        void StreamWriteCheck_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(StreamWriteCheck_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void StreamWriteCheck_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(StreamWriteCheck_i, "status changed");
        }
            
        //Add to StreamWriteCheck.h
...
************************************************************************************************/
int StreamWriteCheck_i::serviceFunction()
{
    bulkio::InLongStream inputStream = input->getCurrentStream();
    if (!inputStream) { // No streams are available
        return NOOP;
    }

    bulkio::LongDataBlock block = inputStream.read(1024);
    if (!block) { // No data available
        // Propagate end-of-stream
        if (inputStream.eos()) {
           outputStream.close();
    ...
    // If there is no output stream open, create one
    if (!outputStream) {
        outputStream = output->createStream(block.sri());
    } else if (block.sriChanged()) {
        // Update output SRI
        outputStream.sri(block.sri());
    }
    ...
    // Write to the output stream
    outputStream.write(&(outputData[0]), 801, block.getTimestamps());
