...
PREPARE_LOGGING(snk_slow_i)

snk_slow_i::snk_slow_i(const char *uuid, const char *label) :
    snk_slow_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

snk_slow_i::~snk_slow_i()
{
}

void snk_slow_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void snk_slow_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &snk_slow_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &snk_slow_i::<callback method>)
        in the constructor.
        ...
        snk_slow_i::snk_slow_i(const char *uuid, const char *label) :
            snk_slow_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &snk_slow_i::scaleChanged);
            addPropertyListener(status, this, &snk_slow_i::statusChanged);
        }

        void snk_slow_i::scaleChanged(float oldValue, float newValue)
        {
            LOG_DEBUG(snk_slow_i, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void snk_slow_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
            LOG_DEBUG(snk_slow_i, "status changed");
        }
...
************************************************************************************************/
int snk_slow_i::serviceFunction()
{
