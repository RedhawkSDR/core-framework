...
PREPARE_LOGGING(stream_src_i)

stream_src_i::stream_src_i(const char *uuid, const char *label) :
    stream_src_base(uuid, label)
{
    // Avoid placing constructor code here. Instead, use the "constructor" function.

}

stream_src_i::~stream_src_i()
{
}

void stream_src_i::constructor()
{
    /***********************************************************************************
...
/***********************************************************************************************
    ...
    Messages:
        ...
        void stream_src_i::my_message_callback(const std::string& id, const my_msg_struct &msg){
        }
        
        Register the message callback onto the input port with the following form:
        this->msg_input->registerMessage("my_msg", this, &stream_src_i::my_message_callback);
    ...
    Properties:
        ...
        addPropertyListener(<property>, this, &stream_src_i::<callback method>)
        in the constructor.
        ...
        //Add to stream_src.cpp
        stream_src_i::stream_src_i(const char *uuid, const char *label) :
            stream_src_base(uuid, label)
        {
            addPropertyListener(scaleValue, this, &stream_src_i::scaleChanged);
            addPropertyListener(status, this, &stream_src_i::statusChanged);
        }

        void stream_src_i::scaleChanged(float oldValue, float newValue)
        {
            RH_DEBUG(this->_baseLog, "scaleValue changed from" << oldValue << " to " << newValue);
        }
            
        void stream_src_i::statusChanged(const status_struct& oldValue, const status_struct& newValue)
        {
...
************************************************************************************************/
int stream_src_i::serviceFunction()
{
    ...
    sri.xdelta = 123;
    sri.blocking = true;
    bulkio::OutFloatStream outputStream = dataFloat_out->createStream(sri);
    //size_t data_size = 1.8e6/sizeof(float);
    size_t data_size = 10;
    for (unsigned int i=0; i<3; i++) {
    	redhawk::buffer<float> outputData(data_size);
