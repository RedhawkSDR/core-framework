...
public class InFileStream extends InStreamBase {

    protected InFilePort _port;
    ...
    protected ArrayDeque<InFilePort.Packet> _queue;
    
    public InFileStream(BULKIO.StreamSRI sri, InFilePort port) {
        /*
        Create an InFileStream.
        ...
        */
        ...
        this._sri = sri;
        this._queue = new ArrayDeque<InFilePort.Packet>();
    ...
    protected void _addTimestamp(FileDataBlock data, double inputOffset, int outputOffset, BULKIO.PrecisionUTCTime time)
    {
    ...
    protected void _setBlockFlags(FileDataBlock block, InFilePort.Packet packet)
    {
    ...
    protected boolean _queuePacket(InFilePort.Packet packet) {
    ...
    protected FileDataBlock _readPacket(boolean blocking) {
        InFilePort.Packet packet = this._fetchNextPacket(blocking);
        ...
        FileDataBlock block = new FileDataBlock(packet.SRI, packet.dataBuffer, sri_flags, packet.inputQueueFlushed);
        // Only add a timestamp if one was given (File does not include one in
    ...
    protected boolean _canBridge(InFilePort.Packet packet) {
        boolean retval = (packet.sriChanged || packet.inputQueueFlushed);
        return (!retval);
    }

    protected InFilePort.Packet _fetchPacket(boolean blocking) {
        return this._fetchNextPacket(blocking);
    }

    protected void _refreshQueue() {
        InFilePort.Packet packet = null;
    ...
    protected InFilePort.Packet _fetchNextPacket(boolean blocking) {
    ...
    protected int _getSriChangeFlags(InFilePort.Packet packet) {
        if (this._newstream) {
