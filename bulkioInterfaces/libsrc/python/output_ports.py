...
class OutPort(BULKIO__POA.UsesPortStatisticsProvider):
    ...
    def connectPort(self, connection, connectionId):
        ...
        try:
           try:
              ...
              if self._portLog:
                  self._portLog.debug('bulkio::OutPort  CONNECT PORT:' + str(self.name) + ' CONNECTION:' + str(connectionId))
              
           except:
              if self._portLog:
                  self._portLog.error('bulkio::OutPort  CONNECT PORT:' + str(self.name) + ' PORT FAILED NARROW')
              raise Port.InvalidPort(1, "Invalid Port for Connection ID:" + str(connectionId))
        finally:
    ...
    def disconnectPort(self, connectionId):
        ...
        if not self.outConnections.has_key(connectionId):
            if self._portLog:
                self._portLog.debug("bulkio::OutPort  disconnectPort() - connectionId " + str(connectionId) + " is not contained in list of outConnections")
            return
        self.port_lock.acquire()
        try:
            ...
            if self._portLog:
                self._portLog.debug( "bulkio::OutPort DISCONNECT PORT:" + str(self.name) + " CONNECTION:" + str(connId))
                self._portLog.trace( "bulkio::OutPort DISCONNECT PORT:" + str(self.name) + " updated sriDict" + str(self.sriDict))
        finally:
            self.port_lock.release()
    ...
    def _pushOversizedPacket(self, data, T, EOS, streamID):
        ...
        for start in xrange(0, len(data), self.maxSamplesPerPush):
            ...
            if self._portLog:
                self._portLog.trace("_pushOversizedPacket() calling pushPacket with pushSize " + str(len(data[start:end])) + " and packetTime twsec: " + str(packetTime.twsec) + " tfsec: " + str(packetTime.tfsec))
...
class OutFilePort(OutPort):
    ...
    def pushPacket(self, URL, T, EOS, streamID):
        ...
        if self._portLog:
            self._portLog.trace('bulkio::OutFilePort  pushPacket ENTER ')
        ...
        if self._portLog:
            self._portLog.trace('bulkio::OutFilePort  pushPacket EXIT ')
...
class OutXMLPort(OutPort):
    ...
    def pushPacket(self, xml_string, EOS, streamID):
        ...
        if self._portLog:
            self._portLog.trace('bulkio::OutXMLPort  pushPacket ENTER ')
        ...
        if self._portLog:
            self._portLog.trace('bulkio::OutXMLPort  pushPacket EXIT ')

    def disconnectPort(self, connectionId):
        if self._portLog:
            self._portLog.trace('bulkio::OutXMLPort  disconnectPort ENTER ')
        if not self.outConnections.has_key(connectionId):
            if self._portLog:
                self._portLog.warn("bulkio::OutXMLPort  disconnectPort() - connectionId " + str(connectionId) + " is not contained in list of outConnections")
            return
        self.port_lock.acquire()
        try:
            connId = str(connectionId)
            ...
            for key,value in self.sriDict.items():
                ...
                self.sriDict[key].connections.discard(connId)
            if self._portLog:
                self._portLog.debug( "bulkio::OutXMLPort DISCONNECT PORT:" + str(self.name) + " CONNECTION:" + str(connId) )
                self._portLog.trace( "bulkio::OutXMLPort DISCONNECT PORT:" + str(self.name) + " updated sriDict" + str(self.sriDict) )
        finally:
            self.port_lock.release()

        if self._portLog:
            self._portLog.trace('bulkio::OutXMLPort  disconnectPort EXIT ')
