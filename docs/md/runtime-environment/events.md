...
An object of the class `Consumer_i` needs to be instantiated and given to the Domain Manager. In Python, the easiest way to access a running domain is to use the REDHAWK runtime package (a thorough description of the REDHAWK runtime package is available in [Runtime Environment Inspection](../runtime-inspection/_index.html)); in the case of a domain called `REDHAWK_DEV`, the process needed to associate this consumer with the event channel is:
